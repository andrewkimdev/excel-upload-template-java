...
public class TariffExemptionDbUniquenessChecker
    implements DatabaseUniquenessChecker<TariffExemptionDto> {

  private final TariffExemptionRepository repository;

  @Override
  public List<RowError> check(
      List<TariffExemptionDto> rows,
      Class<TariffExemptionDto> dtoClass,
    ...
    try {
      ...
      for (int i = 0; i < rows.size(); i++) {
        ...
        boolean exists =
            repository.existsByItemNameAndSpecificationAndHsCode(
                dto.getItemName(), dto.getSpecification(), dto.getHsCode());

        if (exists) {
          int currentRowNum = sourceRowNumbers.get(i);
          int colIndex =
              excelColumn != null && !excelColumn.column().isEmpty()
                  ? ExcelColumnUtil.letterToIndex(excelColumn.column())
                  : -1;
          String colLetter =
              excelColumn != null && !excelColumn.column().isEmpty() ? excelColumn.column() : "?";
